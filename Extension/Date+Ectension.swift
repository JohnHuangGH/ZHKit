...
extension Date {
    /// 获取DateComponents
    func zh_dateComponents(_ comps: Set<Calendar.Component> = [.era, .year, .month, .day, .hour, .minute, .second, .weekday, .timeZone]) -> DateComponents {
        let calendar = Calendar(identifier: .gregorian)
        return calendar.dateComponents(comps, from: self)
    }
    
    /// 获取DateFormatter
    static func zh_dateFormatter(_ format: String, _ timezone: TimeZone! = TimeZone(abbreviation: "GMT+8")) -> DateFormatter{
    ...
    func zh_isSameDay(_ date: Date) -> Bool {
        let comp1 = zh_dateComponents([.year, .month, .day])
        let comp2 = date.zh_dateComponents([.year, .month, .day])
...
//        return Calendar.current.date(from: zeroComp) ?? self
        ...
        let formatter = Date.zh_dateFormatter(ZHDateFormat.Date.rawValue, timezone)
...
extension Date {
    ...
    func zh_12hStr() -> String {
        let hour = self.zh_dateComponents().hour ?? 0
...
extension Date {
    ...
    static func zh_date(with dateStr: String, _ dateFormat: String, _ timezone: TimeZone! = TimeZone(abbreviation: "GMT+8")) -> Date? {
        return Date.zh_dateFormatter(ZHDateFormat.Date.rawValue, timezone).date(from: dateStr)
    }
    /// Date字符串格式化为Date
