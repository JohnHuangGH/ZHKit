...
class ZHNetManager: NSObject {
    ...
    func request(url urlStr: String, params: [String: Any], method: ZHNetMethod, paramsType: ZHNetParamsType = .default, completed: ((_ res: Any?)->Void)?){
        ...
        if method == .get || method == .delete {
            fullURL += "?\(makeParamsString(params: params))"
        }
        ...
        if method == .post || method == .put {
            if paramsType == .default {
                ...
                request.httpBody = makeParamsString(params: params).toData()
            }else{
    ...
    func uplaod(url urlStr: String, fileDict: [String: Data], name: String = "files", params: [String: Any]? = nil, completed: ((_ success: Bool)->Void)?){
        ...
        // body
        let body = crateMultBody(fileDict: fileDict, boundary: kBoundary, name: name, params: params)
        // uploadTask
...
extension ZHNetManager {
    ...
    private func makeParamsString(params: [String: Any]) -> String {
        var str = ""
    ...
    private func crateMultBody(fileDict: [String: Data], boundary: String, name: String, params: [String: Any]?) -> Data {
        // 创建body
        var body = Data()
